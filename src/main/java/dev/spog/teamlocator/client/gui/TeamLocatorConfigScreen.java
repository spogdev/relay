...
public class TeamLocatorConfigScreen extends Screen {
    ...
    private int drawTrust(DrawContext graphics, int left, int top, int right,
        ...
        centered(graphics, Text.translatable("relay.config.column.alerts"), muteX, SWITCH_COL_W, y);
        centered(graphics, Text.translatable("relay.config.column.visibility"), shareX, SWITCH_COL_W, y);
        ...
        for (TrustEntry entry : entries) {
            ...
            drawDangerButton(graphics, removeX, y - 3, 50,
                    Text.translatable("relay.config.remove").getString());
     ...
     */
    private void drawSwitch(DrawContext graphics, int x, int y, boolean on, boolean enabled) {
     ...
     */
    private void drawDangerButton(DrawContext graphics, int x, int y, int boxWidth, String text) {
        int boxHeight = this.textRenderer.fontHeight + 8;
        int fill = 0x50241A1D;
        int border = 0xA0955A5A;
