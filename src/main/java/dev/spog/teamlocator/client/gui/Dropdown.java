...
public class Dropdown<T> {
	 ...
	 * {@code ClickableWidget.active} -- the integrations page disables controls whose mod is absent.
	 */
	...
	public boolean isHovered(TextRenderer textRenderer, int mouseX, int mouseY) {
		return contains(mouseX, mouseY, x, y, width, height(textRenderer));
	}
	...
	public void draw(DrawContext graphics, TextRenderer textRenderer, T current, int mouseX, int mouseY) {
		int boxHeight = height(textRenderer);
	...
	public void drawOverlay(DrawContext graphics, TextRenderer textRenderer, T current, int mouseX, int mouseY) {
		...
		int listTop = listTop(textRenderer, listHeight);
	...
	public boolean click(TextRenderer textRenderer, double mouseX, double mouseY) {
		if (!active) {
			return false;
		}
		int boxHeight = height(textRenderer);
		...
		int visible = Math.min(MAX_VISIBLE, entries.size());
		int listTop = listTop(textRenderer, visible * ROW_HEIGHT + 4);
		for (int i = 0; i < visible; i++) {
			int rowY = listTop + 2 + i * ROW_HEIGHT;
	...
	private int listTop(TextRenderer textRenderer, int listHeight) {
		return y + height(textRenderer) + 2;
	}
	...
	private static String trim(TextRenderer textRenderer, String text, int max) {
		String out = text;
