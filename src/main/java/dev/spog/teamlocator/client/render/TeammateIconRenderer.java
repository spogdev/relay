 ...
 * locator, so a wall between you should not hide it. Uses the same {@code AFTER_ENTITIES} world-render
 * hook and camera/billboard setup as {@link PingRenderer}, which documents the Yarn specifics.
 */
...
public final class TeammateIconRenderer {
    ...
    public static void register() {
        WorldRenderEvents.AFTER_ENTITIES.register(TeammateIconRenderer::render);
    }
    ...
    private static void renderIcon(MinecraftClient mc, MatrixStack matrices,
        ...
        if (entity != null) {
            // getLerpedPos(partial) lerps from the entity's PREVIOUS tick position. A player entity
            // that just entered render range has not ticked yet (age == 0), so its prevX/Y/Z are still
            // zero — lerping through that yanked the icon toward world origin for a frame, which
            // billboarded to a screen corner (the "glitching to the top left"). Only interpolate once
            // the entity has a real previous position; before that, use its current position flat.
            if (entity.age > 0) {
        ...
        int bg = (rgb >> 8) & 0xFF;
        int bb = rgb & 0xFF;
        VertexConsumer border = consumers.getBuffer(RenderLayers.textBackgroundSeeThrough());
     ...
     * skin derived from their UUID while a real one loads. Same resolution order Xaero uses.
     */
    private static Identifier skinTexture(MinecraftClient mc, UUID id) {
        if (mc.world != null) {
        ...
        if (mc.getNetworkHandler() != null) {
            ...
            if (info != null) {
                SkinTextures skin = info.getSkinTextures();
                if (skin != null) {
                    return skin.body().texturePath();
                }
            }
        }
        return DefaultSkinHelper.getSkinTextures(id).body().texturePath();
    }

    /** The name plate: a translucent-black bar sized to the name, then the name centred on it. */
    private static void drawName(MinecraftClient mc, VertexConsumerProvider consumers,
                                 MatrixStack.Entry pose, UUID id, int alpha) {
        ...
        float top = bottom + font.fontHeight;
        int bgAlpha = Math.min(alpha, NAME_BG_ALPHA);
        VertexConsumer plate = consumers.getBuffer(RenderLayers.textBackgroundSeeThrough());
        ...
        font.draw(name, 0.0f, 0.0f, textArgb, false, mat, consumers,
                TextRenderer.TextLayerType.SEE_THROUGH, 0, FULL_BRIGHT);
    }

    /** A flat coloured quad on the see-through nametag-background layer (POSITION_COLOR_LIGHT). */
    private static void colorQuad(VertexConsumer buffer, MatrixStack.Entry pose,
                                  float x0, float y0, float x1, float y1, float z,
                                  int r, int g, int b, int a) {
        buffer.vertex(pose, x0, y0, z).color(r, g, b, a).light(FULL_BRIGHT);
        buffer.vertex(pose, x1, y0, z).color(r, g, b, a).light(FULL_BRIGHT);
        buffer.vertex(pose, x1, y1, z).color(r, g, b, a).light(FULL_BRIGHT);
