...
public class TeamLocatorConfigScreen extends Screen {
     ...
     */
    private final List<net.minecraft.client.gui.components.AbstractWidget> pinnedWidgets =
    ...
    /**
     * Add a scrolled widget if any part of it falls inside the scrolling viewport.
     *
     * <p>Kept while it is <em>partially</em> visible, so a row entering or leaving the viewport is
     * clipped at the bar's edge rather than vanishing whole — the widget drawing in
     * {@link #extractRenderState} is wrapped in a scissor for exactly that. Only rows completely
     * outside the viewport are dropped.
     *
     ...
     */
    private <T extends net.minecraft.client.gui.components.AbstractWidget> T addPinned(T widget) {
     ...
     */
    private void renderPinnedWidgets(GuiGraphicsExtractor graphics, int mouseX, int mouseY,
                                     float delta) {
    ...
    private void initSettingsPage(int cx) {
        ...
        addScrolled(sy(HUD_ROW_6_Y), described(CycleButton.onOffBuilder(config.hudShortenCoords)
                .create(cx - 205, sy(HUD_ROW_6_Y), 200, 20,
                        Component.translatable("relay.config.hud_shorten_coords"),
                        (btn, value) -> {
        ...
        addScrolled(sy(MARKER_ROW_Y), described(CycleButton.onOffBuilder(config.playerMarkersEnabled)
                .create(cx - 205, sy(MARKER_ROW_Y), 200, 20,
                        Component.translatable("relay.config.player_markers_enabled"),
                        (btn, value) -> {
        ...
        markerSize.setTooltip(Tooltip.create(Component.translatable(xaeroDrawsMarkers
        ...
        markerOpacity.setTooltip(Tooltip.create(Component.translatable(xaeroDrawsMarkers
                ? "relay.config.player_marker_size.xaero"
        ...
        markerHide.setTooltip(Tooltip.create(Component.translatable(xaeroDrawsMarkers
                ? "relay.config.player_marker_size.xaero"
        ...
        var showDistance = described(CycleButton.onOffBuilder(config.playerMarkerShowDistance)
                .create(cx - 205, sy(MARKER_ROW_3_Y), 200, 20,
                        Component.translatable("relay.config.player_marker_show_distance"),
                        (btn, value) -> {
    ...
    private void initIntegrationsPage(int cx) {
        ...
        var useXaeroIcons = CycleButton.onOffBuilder(config.useXaeroInWorldIcons)
                .create(cx + 5, sy(INTEGRATIONS_ROW_Y), 200, 20,
                        Component.translatable("relay.config.xaero_markers"),
                        (btn, value) -> {
                            config.useXaeroInWorldIcons = value;
                            config.save();
                        });
        useXaeroIcons.active = minimap;
        useXaeroIcons.setTooltip(Tooltip.create(Component.translatable(
    ...
    private void initBottomBar(int cx) {
        ...
        addPinned(Button.builder(Component.translatable("relay.config.done"),
     ...
     * inside. Part of the General page's Advanced section, so it scrolls with the rest.
     *
     ...
     * old widget behaviour, which left the frame vanishing while the EditBox inside it clipped — the
     * two halves of one control behaving differently.
     */
    ...
    private void drawSettingsLabels(GuiGraphicsExtractor graphics, int cx) {
        ...
        graphics.text(this.font, Component.translatable("relay.config.section.player_marker"),
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        // still focus the address field — the whole frame reads as one text box. The frame scrolls
        // with the General page's Advanced section, so the hit test follows it.
