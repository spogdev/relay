...
public class TeamHud implements HudElement {
    ...
    public void render(DrawContext graphics, RenderTickCounter delta) {
        ...
        // First pass: build the rows so we know their widths before placing anything.
        ...
        for (TrackedPos e : entries) {
            // Absent PlayerListEntry means the teammate is not in OUR tab list — which on a proxy
            // network routinely means "on a different backend", not "gone". Dropping the row here was
            // why the HUD could sit empty while the in-world marker drew the same teammate perfectly
            // well: the marker falls back to a default skin and a stored name, and the HUD now does too.
            ...
            SkinTextures skin = info != null
                    ? info.getSkinTextures() : DefaultSkinHelper.getSkinTextures(e.id());
